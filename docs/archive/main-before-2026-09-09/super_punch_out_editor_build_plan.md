...
## Phase 1 — ROM Core + Project Skeleton
**Goal:** open ROM, validate, save project

Tasks:
- build ROM validator
- implement file open/save
- detect headered/headerless
- implement project file format
- implement patch output base

Deliverable:
- app opens ROM and shows boxer list

## Phase 2 — Safe Asset Editing
**Goal:** ship usable first version

Tasks:
- palette editor
- icon/portrait export/import
- preview panels
- warning system
- export modded ROM / IPS

Deliverable:
- first public/private usable build

## Phase 3 — Graphics Bin Editing
...
- tile import/export
- per-bin diff
- compressed asset reinsertion
- shared-bank edit handling

Deliverable:
- graphics modding becomes practical

## Phase 4 — Curated Fighter Preview Layouts
**Goal:** make graphics editing pleasant

Tasks:
- build boxer layout metadata
- reconstruct common sheet/frame views
- add image preview assembly where known

Deliverable:
- less raw tile pain, more visual editing

## Phase 5 — Script Awareness
**Goal:** expose logic layer

Tasks:
- link scripts to boxers/assets
- read-only script viewer
- safe parameter editing for known fields

Deliverable:
- first behavior-side editing support

## Phase 6 — Advanced Mutation Features
...
Deliverable:
- genuinely powerful game-specific editor
...
1. ROM validator
2. Boxer browser using current manifest
3. Palette viewer/editor
4. Icon export/import
5. Large portrait export/import
6. Save-as ROM + IPS export
7. Shared-bank warning engine
8. Raw fighter bin viewer
9. PNG/tile import for unique fighters
10. Compressed bin reinsertion
11. Curated boxer preview layouts
12. Script viewer
...
- 4bpp tile decode/encode
- compression round-trip where supported
- IPS patch generation

## Integration tests
- open clean ROM and load manifest
- export unchanged ROM copy
- palette edit then save
- portrait import then save
- unique fighter graphics bin edit then save
- shared bank edit triggers warning
...
- user can open the correct ROM
- boxer list loads correctly
- every boxer shows palette/icon/portrait where present
- palette edits preview correctly
- icon/portrait imports validate correctly
- export creates working ROM copy
- shared assets are visibly marked
- IPS export works on changed data

## V2 acceptance criteria
- user can inspect fighter bins visually
- unique fighter bin edits can be imported and saved
- compressed asset handling is reliable
- diff view shows changed bins
- validation blocks bad imports cleanly

## V3 acceptance criteria
- script references are visible for supported fighters
- read-only script inspection works
- known-safe script parameters can be changed without corrupting build
