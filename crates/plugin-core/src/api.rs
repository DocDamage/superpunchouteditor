...
impl PluginApi {
    pub fn new(context: Arc<RwLock<PluginContext>>) -> Self {
        Self {
            context,
            rom_reader: Box::new(|_, _| {
                Err(PluginError::ApiError("ROM reader not initialized".into()))
            }),
            rom_writer: Box::new(|_, _| {
                Err(PluginError::ApiError("ROM writer not initialized".into()))
            }),
            asset_getter: Box::new(|_| {
                Err(PluginError::ApiError("Asset getter not initialized".into()))
            }),
