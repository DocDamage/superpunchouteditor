...
use crate::types::*;
use crate::{
    EditorEvent, EditorPlugin, PluginConfig, PluginContext, PluginError, PluginInfo,
    PluginMenuItem, PluginResult, PluginToolbarButton,
};
...
impl PluginManager {
    ...
    pub fn load_plugin<P: AsRef<Path>>(&self, path: P) -> PluginResult<PluginInfo> {
        ...
        let mut plugin: Box<dyn EditorPlugin> =
            if path.extension().and_then(|s| s.to_str()) == Some("lua") {
    ...
    pub fn broadcast_event(&self, event: &EditorEvent) -> PluginResult<()> {
        let plugins_to_notify: Vec<String> = {
            ...
            plugins
                .iter()
                .filter(|(id, _)| configs.get(*id).map(|c| c.enabled).unwrap_or(false))
                .map(|(id, _)| id.clone())
                .collect()
        };
...
impl ScriptRunner {
    ...
    pub fn run_string(&self, script: &str, name: Option<&str>) -> PluginResult<ScriptResult> {
        ...
        // Execute the script
        let result = lua.load(script).set_name(name.unwrap_or("script")).exec();
