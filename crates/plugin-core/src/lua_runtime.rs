...
use crate::types::*;
use crate::{
    EditorEvent, EditorPlugin, PluginCommand, PluginContext, PluginError, PluginInfo, PluginResult,
    PluginType,
};
use std::path::Path;
...
enum LuaMessage {
    ...
    ExecuteCommand(
        String,
        serde_json::Value,
        Sender<PluginResult<serde_json::Value>>,
    ),
...
impl LuaPlugin {
    ...
    pub fn from_file<P: AsRef<Path>>(path: P, api: Arc<PluginApi>) -> PluginResult<Self> {
...
impl EditorPlugin for LuaPlugin {
    ...
    fn on_event(&mut self, event: &EditorEvent, _ctx: &PluginContext) -> PluginResult<()> {
        self.tx
            .send(LuaMessage::OnEvent(event.clone()))
            .map_err(|_| PluginError::PluginCrashed("Worker thread died".into()))?;
        Ok(())
    }
    ...
    fn execute_command(
        ...
        let (tx, rx) = channel();
        self.tx
            .send(LuaMessage::ExecuteCommand(command.into(), args.clone(), tx))
            .map_err(|_| PluginError::PluginCrashed("Worker thread died".into()))?;
...
impl LuaWorker {
    ...
    fn load_script(&mut self, path: &Path) -> PluginResult<()> {
        ...
        let (id, name, version, author, description, api_version) = {
            let info_table: mlua::Table = self
                .lua
                .globals()
                .get("PLUGIN_INFO")
                .map_err(|_| PluginError::LuaError("Missing PLUGIN_INFO table".into()))?;

            let id: String = info_table
                .get("id")
                .map_err(|_| PluginError::LuaError("Missing PLUGIN_INFO.id".into()))?;
            let name: String = info_table
                .get("name")
                .unwrap_or_else(|_| "Unnamed Plugin".into());
            let version: String = info_table.get("version").unwrap_or_else(|_| "0.1.0".into());
            let author: String = info_table
                .get("author")
                .unwrap_or_else(|_| "Unknown".into());
            let description: String = info_table.get("description").unwrap_or_else(|_| "".into());
            let api_version: u32 = info_table.get("api_version").unwrap_or(1);
        ...
        // ROM read function
        {
            ...
            api_table
                .set(
                    "rom_read",
                    self.lua
                        .create_function(move |_, (offset, length): (usize, usize)| {
        ...
        // ROM write function
        {
            ...
            api_table
                .set(
                    "rom_write",
                    self.lua
                        .create_function(move |_, (offset, data): (usize, mlua::String)| {
        ...
        // ROM read byte
        {
            ...
            api_table
                .set(
                    "rom_read_byte",
                    self.lua
                        .create_function(move |_, offset: usize| {
        ...
        // ROM write byte
        {
            ...
            api_table
                .set(
                    "rom_write_byte",
                    self.lua
                        .create_function(move |_, (offset, value): (usize, u8)| {
        ...
        // Address conversion
        {
            ...
            api_table
                .set(
                    "snes_to_pc",
                    self.lua
                        .create_function(move |_, (bank, addr): (u8, u16)| {
...

        {
            ...
            api_table
                .set(
                    "pc_to_snes",
                    self.lua
                        .create_function(move |_, pc: usize| Ok(api.pc_to_snes(pc)))
                        .map_err(|e| PluginError::LuaError(e.to_string()))?,
                )
                .map_err(|e| PluginError::LuaError(e.to_string()))?;
        }
        ...
        // Pattern search
        {
            ...
            api_table
                .set(
                    "find_pattern",
                    self.lua
                        .create_function(move |_, pattern: mlua::String| {
        ...
        while let Ok(msg) = self.rx.recv() {
            match msg {
                ...
                LuaMessage::Shutdown => {
                    ...
                    if let Ok(on_shutdown) =
                        self.lua.globals().get::<_, mlua::Function>("on_shutdown")
                    {
                        let _ = on_shutdown.call::<_, ()>(());
                    }
                    break;
                }
                LuaMessage::OnEvent(event) => match event {
                    EditorEvent::RomLoaded => {
                        if let Ok(handler) =
                            self.lua.globals().get::<_, mlua::Function>("on_rom_loaded")
                        {
                    ...
                    EditorEvent::RomSaving => {
                        if let Ok(handler) =
                            self.lua.globals().get::<_, mlua::Function>("on_rom_saving")
                        {
                            let _ = handler.call::<_, ()>(());
                        }
                    }
                    EditorEvent::AssetModified => {
                        if let Ok(handler) = self
                            .lua
                            .globals()
                            .get::<_, mlua::Function>("on_asset_modified")
                        {
                            let _ = handler.call::<_, ()>(());
                        }
                    }
                    EditorEvent::PaletteEdited => {
                        if let Ok(handler) = self
                            .lua
                            .globals()
                            .get::<_, mlua::Function>("on_palette_edited")
                        {
                            let _ = handler.call::<_, ()>(());
                        }
                    }
                    EditorEvent::SpriteEdited => {
                        if let Ok(handler) = self
                            .lua
                            .globals()
                            .get::<_, mlua::Function>("on_sprite_edited")
                        {
                            let _ = handler.call::<_, ()>(());
                        }
                    }
                    EditorEvent::AnimationEdited => {
                        if let Ok(handler) = self
                            .lua
                            .globals()
                            .get::<_, mlua::Function>("on_animation_edited")
                        {
                            let _ = handler.call::<_, ()>(());
                        }
                    }
                    EditorEvent::ProjectCreated => {
                        if let Ok(handler) = self
                            .lua
                            .globals()
                            .get::<_, mlua::Function>("on_project_created")
                        {
                            let _ = handler.call::<_, ()>(());
                        }
                    }
                    EditorEvent::ProjectOpened => {
                        if let Ok(handler) = self
                            .lua
                            .globals()
                            .get::<_, mlua::Function>("on_project_opened")
                        {
                            let _ = handler.call::<_, ()>(());
                        }
                    }
    ...
    fn execute_command(
        ...
        if let Ok(commands) = self.lua.globals().get::<_, mlua::Table>("COMMANDS") {
            if let Ok(func) = commands.get::<_, mlua::Function>(command) {
                ...
                let result: mlua::Value = func
                    .call(lua_args)
                    .map_err(|e| PluginError::LuaError(e.to_string()))?;
                return lua_to_json(&result).map_err(|e| PluginError::LuaError(e.to_string()));
            }
        }
        ...
        if let Ok(func) = self.lua.globals().get::<_, mlua::Function>(command) {
            let lua_args =
                json_to_lua(&self.lua, args).map_err(|e| PluginError::LuaError(e.to_string()))?;
            let result: mlua::Value = func
                .call(lua_args)
                .map_err(|e| PluginError::LuaError(e.to_string()))?;
            return lua_to_json(&result).map_err(|e| PluginError::LuaError(e.to_string()));
